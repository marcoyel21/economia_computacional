...
Es evidente que casi todas estas variables tienen outliers. Por ello, conviene imputar la la mediana en lugar de la media.

**phones, models y eqpdays**

En este caso, solo estamos hablando de un missing value y más de la mitad de las observaciones igual a 1 para _phones_ y _models_. Podemos imputarles la mediana.
...
Estas variables muestran una distribución normal, por lo que también puede aplicárseles la imputación con la mediana.

```{r}
data$revenue[is.na(data$revenue)] <- median(data$revenue[!is.na(data$revenue)])
data$mou[is.na(data$mou)] <- median(data$mou[!is.na(data$mou)])
data$recchrge[is.na(data$recchrge)]<- median(data$recchrge[!is.na(data$recchrge)])
```

 **roam y directas**

Estas variables están altamente concentradas (más del 50% de las observaciones en 0 o entre 0 y 1, respectivamente). Por lo tanto, también les imputamos la mediana.

```{r}
data$directas[is.na(data$directas)] <- median(data$directas[!is.na(data$directas)])
data$roam[is.na(data$roam)] <- median(data$roam[!is.na(data$roam)])
```
...
```{r}
# temporal
data$overage[is.na(data$overage)] <- median(data$overage[!is.na(data$overage)])

# numero de filas con NAs
data[!complete.cases(data),] %>% nrow()

```
Ahora tenemos una base sin valores faltantes.
...
## Análisis general de missing values (pendiente)
...
data$phones[is.na(data$phones)] <- mode(factor(data$phones[!is.na(data$phones)])) %>% as.numeric
data$models[is.na(data$models)] <- mode(factor(data$models[!is.na(data$models)])) %>% as.numeric
...
En este caso podemos cortar la variable para tener datos más centrados y poder imputar con mayor precisión (ABIERTO A DISCUSCION): debido a la distribución de los datos podemos optar por dos caminos A) imputar la media muestral o B) realizar un supuesto e imputar 0, pues se trata de cambios y existe evidencia suficiente para sotener que la media es muy cercana a cero.

```{r}
data$changem[is.na(data$changem)] <- 0 #mean((data$changem[!is.na(data$changem)]))
data$changer[is.na(data$changer)] <- 0 #mean((data$changer[!is.na(data$changer)]))
```
...
En esta tabla podemos observar que en general los  modelos nos dan una predicción mejor a la media, excepto en el ultimo modelo. Esta información nos da luz verde para realizar una imputación con base en una regresión lineal.
...
data$models <- as.numeric(data$models)
...
#A
lasso<-predict(cvlasso_a,
              newdata = data_validation[,-1],
              type='link',
               select = "min")
lasso<-as.numeric(y_pred_a)

cv_lasso_a
#dataframe
A<-data.frame(data_validation$customer, churn_validation, lasso)
...
tree_score_predict_a <- predict(tree_cut_a,newdata=data_validation)
tree_class_predict_a <- predict(tree_cut_a,newdata=data_validation,type="class")

# unir con base de predicciones
A <- data.frame(A,tree_score_predict_a[,1],tree_class_predict_a)
names(A)[4] <- "tree_score_predict"
```
