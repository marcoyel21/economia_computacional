...
smote <- smotefamily::SMOTE(data %>% na.exclude, (data %>% na.exclude)$churn, K=10, dup_size=0)
...
# la verdad aquí me atoré, no sé si el siguiente paso ahora sí es volver a hacer sampling
# va la prueba, no sé si sea correcto

training_size <- 0.8

training_rows <- sample(seq_len(nrow(oversampled)),size=floor(training_size*nrow(oversampled)))

data_training <- oversampled[training_rows,]

data_validation <- oversampled[-training_rows,]
