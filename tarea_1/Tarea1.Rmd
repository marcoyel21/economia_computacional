...
elast_sabores <- lm(log(quantity+1)~log(unit_price+1)*relevel(as.factor(sabores_pop),ref='otros'),
                    data=base)
...
ggplot(data =fdr, aes(x=indice,y=p.value))+
  ...
  geom_abline(slope=q/length(orden$p.value),col='red')+
  theme_bw()+
  labs(title="Histograma de p values (zoom in p<0.05)") + theme_economist() + scale_fill_economist()+
  ylim(c(0,0.05))
...
Antes 56 variables salían significativas. Después, con FDR solo 30.
```{r}
# Antes
