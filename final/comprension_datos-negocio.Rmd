...
Un punto muy evidente de la exploración visual es que unas estaciones tienen mayor tráfico que otras. Para identificar a estas estaciones, hacemos un mapa de calor de la frecuencia de viajes por estaciones de origen y destino.
...
# antigüedad
trips_filter$bike_antiq <- substr(trips_filter$bike_nr %>% as.character(),2,6) %>% as.numeric()

summary(trips_filter$bike_antiq)

# sustituyo la mediana para la antigüedad de las bicis sin bike nr
trips_filter$bike_antiq[is.na(trips_filter$bike_antiq)] <- median(
  trips_filter$bike_antiq, na.rm=T)

# elimino de la base la variable original
trips_filter$bike_nr <- NULL
```
