...
packages <- c(
  ...
  "stargazer",
  "gridExtra",
  "igraph","tidygraph","ggraph","networkD3","visNetwork", "reshape2"
  )
...
Se decide eliminar la variable "Quality_Flag", pues esta contiene únicamente NAs. De manera similar, se elimina "Measurement_Flag", pues su elevado número de NAs, así como la poca información que comunican los únicos valores que toma cuando sí es observado ("T" y "g").
...
weather_calendar$start_date <- unique(trips$start_date)
...
trips[is.na(trips$HPCP), 'HPCP'] <- 'SR'
trips[is.na(trips$rained), 'rained'] <- 'SR'

# Se transforman a factor
trips$HPCP <- trips$HPCP %>% as.factor
