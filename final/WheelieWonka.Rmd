...
# Se asigna también Measurement_Flag
...
Primero separamos la base de validación y de entrenamiento
```{r}
#set.seed(1990)
# usaremos un set de entrenamiento del 80%
#trips_filter <-
#  trips_filter %>%
#  mutate(training_sample = rbinom(n = nrow(trips_filter), 1, 0.8))

# la division de entrenamiento y validación se hará por día.
...
# si queremos estratificar por alguna variable
treat_id <- treatment_assign(data = dates, share_control = 0.2,
                    n_t = 1,
                    strata_varlist = c("start_month","start_year",
                                       "start_year"), 
                    missfits = "global",
...
unique(trips_filter$bike_nr)

trips_filter$bike_fam <- substr((trips_filter$bike_nr) %>% as.character(),1,1) %>%
  as.factor

trips_filter$bike_nr2 <- substr(trips_filter$bike_nr %>% as.character(),2,6) %>% as.numeric()

summary(trips_filter$bike_nr2)

# sustituyo la mediana para las bicis sin bike nr
trips_filter$bike_nr2[is.na(trips_filter$bike_nr2)] <- median(trips_filter$bike_nr2, na.rm=T)
...
train <-  train %>% select(-c("treat","seq_id","bike_nr"))


# validacion
test <- trips_filter[trips_filter$treat==0,] %>%
  select(-c("treat","duration"))

y_test <- trips_filter[trips_filter$treat==0,] %>%
  select(c("seq_id","duration"))
...
random_forest <- map(c(300,400,450,500,550),
                     function(z)
                       ranger(duration~.,
                              data=train,
                              num.trees = z,
...
error_prediccion<-tibble(trees = c(300,400,450,500,550),
...
# predicciones OOS
pred_rf<-predict(random_forest[[4]], data = test)

y_test$pred_rf <- pred_rf$predictions
...
Derivado de estas tablas, se nos ocurrio que una forma muy util de probar el desempeño de los modelos es a través del error del flujo en determinado punto del día. Esto pensando en que en las empresas de este tipo, hay un punto de corte en el que las estaciones se tienen que llenar o vaciar de bicicletas. Para este ejemplo, propusimos un punto de corte a las 12 de la noche, pero bien podría ser en la madrugada, o en la tarde. En la siguiente tabla se muestra el error promedio en el cálculo del flujo de bicis para la hora de corte. 
