...
x_train <- sparse.model.matrix(duration ~.+0, train)
y_train <- train$duration
   
lasso <- cv.gamlr(x = x_train,
                  y = y_train,
                  verb = T,
                  nfold = 10)
...
df_imp <- data.frame(names=(importance(random_forest) %>%
names()),importance=(importance(random_forest)))
