...
trips$start_date <- as.Date(startDate, '%m/%d/%Y')
trips$start_year <- format(trips$start_date, '%Y')
trips$start_month <- format(trips$start_date, '%m')
trips$start_weekday <- weekdays(trips$start_date)

startTime <- sapply(startDateTime, "[", 2)
trips$start_hour <- format(as.POSIXct(startTime,format="%H:%M:%S"),"%H")
...
FVG_comentario: también tendremos que decidir cómo tratar valores vacíos de "gender" y de "zip_code" (como el 29.93%). De la tabla de abajo noten cómo los valore vacíos de "gender" corresponden en su totalidad a los usuarios no registrados, y sólo unos pocos adicionales (155) son usuarios registradas pero sin reportar su "zip_code". De momento les imputo unknown, y muestro una tabla con porcentajes de valores vacíos en ambas.
