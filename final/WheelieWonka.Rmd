...
trips_filter <- trips %>% select(-c("hubway_id","start_DateTime","end_DateTime",
                    "birth_date","end_date","end_year","end_month","end_weekday",
                    "end_hour","end_minute"))


summary(trips_filter)

# imputamos la mediana a la edad
trips_filter$age[is.na(trips_filter$age)] <- median(trips_filter$age, na.rm=T)
```
...
#treat_id <- treatment_assign(data = trips, share_control = 0.2,
#                    n_t = 1,
#                    strata_varlist = c(""), missfits = "global",
#                    seed = 2020, key = "seq_id")$data

# separamos las bases
# a la de validación le dejo el seq_id a las x y a la Y para unir resultados por id
train <- trips_filter[trips_filter$training_sample==1,] %>%
  select(-c("training_sample","seq_id"))

#y_train <- trips$duration[trips$training_sample==1]

test <- trips_filter[trips_filter$training_sample==0,] %>%
  select(-c("training_sample","duration"))

y_test <- trips_filter[trips_filter$training_sample==0,] %>%
...
trips$distance <- distGeo(as.matrix(cbind(trips$start_lng, trips$start_lat)),
