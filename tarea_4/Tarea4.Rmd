...
# para esta parte primero voy estandarizar los datos para reducir el tiempo de computo:
...
#transformo la base de datos a estandarizada para reducir el tiempo de calculo
...
```{r}
#_____________________
#IDENTIFICACIÓN DE MAYORES CORRELACIONES
#____________________
# dummy si la columna x tiene alguna correlación mayor a 0.95 en valor absoluto
...
#______________________
# evalumos balance de tratamiento
...
# la muestra está balanceada? --> NO
undersampling_training %>% group_by(mailing_indicator) %>%
  dplyr::summarise(frecuencia_absoluta=n()) %>% 
  mutate(frecuencia_relativa = frecuencia_absoluta/sum(frecuencia_absoluta)) %>%
  kable(col.names = c("Mailing_indicator",
                      "Frecuencia absoluta",
...
#aqui bautizo como set_training_original al set que vaya a usar
...
#_________________
#LASSO CROSSFITING EN ENTRENAMIENTO
#_________________
...
#_________________
#ESTIMACIÓN DE T Y SPEND EN VALIDATION
#_________________
...
Realizo pruebas de hipótesis para evaluar si el efecto es el mismo
...
```{r}
#para estimar el cate simplemente calculamos con el modelo para cada observación su control y tratamiento, es decir la predicción de su valor observado y su contrafactual.
...
ggplot(data=predicciones_is)+
  geom_histogram(aes(x = tau_hat_cf_is, y = stat(density)), position = "identity",binwidth=1)+
  geom_vline(aes(xintercept=0),
            color="red", linetype="dashed", size=.5)+
  xlim(-10,30)+
  theme_bw()+
...
ggplot(data=predicciones_out)+
  geom_histogram(aes(x = tau_hat_cf_out, y = stat(density)), position = "identity",binwidth=1)+
  geom_vline(aes(xintercept=0), color="red", linetype="dashed", size=.5)+
  xlim(-10,30)+
  theme_bw()+
...
barplot(chart_data$tau,main="Impacto de tratamiento",
        xlab="Décil del score",
        ylab="Impacto")
barplot(chart_data$score_mean, main="Score del modelo (Y)",
        xlab="Décil del score",
        ylab="Promedio del score")
