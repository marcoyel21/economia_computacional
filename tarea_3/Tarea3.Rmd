...
* La edad (dividida por categorías para distintos rangos), ya que probablemente usuarios más jóvenes interactúan mejor con las aplicaciones. Notamos que cumple con los mismos puntos respecto a las regresiones que la variable anterior. Nótese que posiblemente sea una buena idea formar rangos más amplios para no tener que pulverizar tanto las observaciones (en este caso se utilizan cuartiles).
* El valor del telefono, ya que podría servir como un proxy para medir el nivel de ingresos del individuo, por lo que cabría pensar que un usuario con mayor ingreso tienda a realizar más compras y estar más activo en las aplicaciones para tal fin. Nuevamente, la variable es significativa y de relativa importancia.
...
**Cyn:Duda sobre si months since register no podría ser coherente que esté correlacionada… sale significativa en el análisis de la regresión, lo mismo gender F.
...
# FVG: Yo quitaría gender. No veo intuitivamente un motivo para esa variable
#      Igualmente agregaría en su lugar la estratificación por edad que ya cree arriba (age_group)

#CYN: Duda sobre si en la asignación aleatoria no tiene que ser n_t=8 y no 7.
#Duda sobre si "months since register" no valdría ponerla en la estratificación
...
```{r}
#CYN. Duda: Si en la pregunta 5 se elige la opción de francisco de incluir la edad por estrato y no el género en la estratificación, habría que modificar este código. Aunque no pasa nada si se mantiene la variable de género porque, aunque se elijan variables no correlacionadas, como es el caso de esa variable, sólo no se ganaría tanta eficiencia, entonces podría dejarse.  
...
+ probabilidad de que un nuevo usuario sea activo (activos/inactivos)
...
num_compras <- clv/ticket # numero de compras del cliente promedio
...
# suponemos que las ventas ya descontaron la promoción del monto total (monto post descuento)
...
grid.arrange(row161,row162, ncol=1,
             top = "Efectos Promedio de Intención a Tratar por Verificación del Teléfono",
             bottom = "Se controla por edad, sexo, compras totales y meses desde el registro")
```
